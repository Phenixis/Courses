...
export async function addProductAndDefaultPrice(previousState, formData) {
    const { name, price, currency } = Array.from(formData.entries()).reduce(
        (acc, [key, value]) => {
            acc[key] = value;
            return acc;
        },
    ...
    const priceResponse = await addPrice(
        ...
        trial_period_days && !isNaN(trial_period_days)
            ? trial_period_days
            : undefined
    );
...
export async function editProductAndDefaultPrice(previousState, formData) {
    const { productId, name, description, price, currency, priceId } =
    ...
    if (priceId) {
        const actualDefaultPrice = await stripe.prices.retrieve(priceId);
        if (
            !actualDefaultPrice ||
            actualDefaultPrice.unit_amount !==
                Math.round(parseFloat(price) * 100) ||
            actualDefaultPrice.currency !== currency ||
            (interval &&
                interval !== "one-time" &&
                (!actualDefaultPrice.recurring ||
                    actualDefaultPrice.recurring.interval !== interval ||
                    actualDefaultPrice.recurring.trial_period_days !==
                        trial_period_days)) ||
            ((!interval || interval === "one-time") &&
                actualDefaultPrice.recurring)
        ) {
            priceResponse = await addPrice(
                productResponse.productId,
                Math.round(parseFloat(price) * 100),
                currency,
                interval && interval !== "one-time" ? interval : undefined,
    ...
    if (priceResponse !== undefined) {
        ...
        const productStatusResponse = await editProduct(
            productResponse.productId,
            name,
            formData.get("description") != ""
                ? formData.get("description")
        ...
        const priceStatusResponse = await togglePriceStatus(
            formData.get("priceId"),
            true
        );
        ...
        if (formData.get("migrate") === "on") {
            const migrateResponse = await migrateSubscriptions(
